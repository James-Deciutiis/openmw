...
namespace Constants
{
    ...
    constexpr std::string NightDayLabel = "NightDaySwitch";

    // A label to mark visual switches for herbalism feature
    constexpr std::string HerbalismLabel = "HerbalismSwitch";

    // Percentage height at which projectiles are spawned from an actor
    constexpr float TorsoHeight = 0.75f;

    constexpr float sStepSizeUp = 34.0f;
    constexpr float sMaxSlope = 46.0f;

    // Identifier for main scene camera
    constexpr std::string SceneCamera = "SceneCam";
